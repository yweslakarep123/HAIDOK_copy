...
def send_email(to_email, subject, body):
    ...
    SENDER_PASSWORD = "password_app_gmail_anda" # Gunakan App Password jika pakai Gmail
...
def test_notification():
    ...
    title = f'Pengingat Tes: {schedule.nama_obat}'
    body = f'Ini adalah notifikasi tes untuk jadwal minum obat {schedule.nama_obat}.'

    if method == 'email':
        if not user.email:
            return jsonify({'success': False, 'error': 'User tidak memiliki email terdaftar'}), 400
        send_email(user.email, title, body)
        return jsonify({'success': True})


    return jsonify({'success': False, 'error': 'Metode tidak valid'}), 400

def send_medication_reminder():
    with app.app_context():
        now = datetime.utcnow()
        schedules = MedicationSchedule.query.filter_by(aktif=True).all()
        for sched in schedules:
            last = sched.last_notified_at or sched.waktu_mulai
            next_time = last + timedelta(hours=sched.interval_jam) if sched.last_notified_at else sched.waktu_mulai
            if now >= next_time:
                user = User.query.get(sched.user_id)
                if user:
                    title = f'Pengingat Minum Obat: {sched.nama_obat}'
                    body = f'Sudah waktunya minum obat {sched.nama_obat}. {sched.catatan or ""}'
                ...
                sched.last_notified_at = now
                db.session.commit()

# Scheduler start
scheduler = BackgroundScheduler(daemon=True)
scheduler.add_job(send_medication_reminder, 'interval', minutes=1)
scheduler.start()
